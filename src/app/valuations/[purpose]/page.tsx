    ...
    <>
      ...
      <section className="bg-green text-cream">
        <Container className="py-12 lg:py-16">
          ...
          <h1 className="max-w-[24ch] text-[clamp(2rem,4.6vw,3.25rem)] text-cream">
            Valuation {purpose.name.toLowerCase()}
          </h1>
          <p className="mt-5 max-w-[42ch] font-display text-[clamp(1.25rem,2.4vw,1.6rem)] leading-snug text-cream/90">
            {purpose.situation}
          </p>
        </Container>
      </section>
      ...
      <section className="py-12 lg:py-16">
        <Container className="grid gap-10 lg:grid-cols-[1.55fr_1fr] lg:gap-12">
          <div>
            <h2 className="text-[1.5rem] text-green">What the figure is based on</h2>
            ...
            <dl className="mt-6">
              {purpose.basis.map((entry) => (
                <div key={entry.term} className="border-t border-rule py-4">
                  <dt className="font-display text-[1.0625rem] text-green">{entry.term}</dt>
                  <dd className="mt-1.5 max-w-[62ch] text-[0.9375rem] leading-relaxed text-muted">
                    {entry.meaning}
                  </dd>
                </div>
              ))}
            ...
            <div className="mt-10">
              <h2 className="text-[1.5rem] text-green">What you receive</h2>
              <dl className="mt-4">
            ...
            <div className="mt-10">
              <h2 className="text-[1.5rem] text-green">Which assets we value for this</h2>
              <ul className="mt-4 grid gap-3 sm:grid-cols-2">
                {purpose.assets.map((assetSlug) => {
                  ...
                  return (
                    ...
                    >
                      <h3 className="font-display text-[1.0625rem] text-green">{asset.name}</h3>
          ...
          */}
          <aside className="lg:sticky lg:top-[calc(var(--header-h)+16px)] lg:self-start">
            <div className="border border-rule border-t-2 border-t-gold bg-paper p-5">
              <p className="text-[0.8125rem] text-muted">Typical turnaround</p>
              <p className="tnum mt-1 font-display text-[2rem] leading-none text-green">
                {purpose.turnaround}
              </p>
            ...
            <div className="mt-6 border border-green/25 bg-green/8 p-5">
              <h2 className="font-display text-[1.125rem] text-green">Who would sign it</h2>
              <p className="mt-2 text-[0.875rem] leading-relaxed text-muted">
          ...
          <ul className="mt-8 border-t border-rule">
            {others.map((item) => (
              <li key={item.slug}>
                ...
                >
                  <span className="font-display text-[1.0625rem] text-green group-hover:text-gold-deep">
                    {item.name}
                  </span>
                  <span className="max-w-[52ch] text-[0.875rem] leading-snug text-muted">
                    {item.situation}
                  </span>
                </Link>
              </li>
